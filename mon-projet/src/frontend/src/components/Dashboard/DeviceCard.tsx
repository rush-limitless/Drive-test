...
const DeviceCard: React.FC<DeviceCardProps> = ({ device, onSelect, isSelected }) => {
  ...
  const handleTest = async (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const response = await fetch(`http://localhost:8007/api/v1/devices/${device.id}/test`, {
        method: 'POST'
      });
  ...
  const handleReboot = async (e: React.MouseEvent) => {
    ...
    if (confirm(`Reboot device ${device.id}?`)) {
      try {
        const response = await fetch(`http://localhost:8007/api/v1/devices/${device.id}/reboot`, {
          method: 'POST'
        });
        const result = await response.json();
        alert(result.message);
