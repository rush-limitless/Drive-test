...
def _project_root() -> Path:
    """Resolve the project root by looking for the specs/modules directory."""
    current = Path(__file__).resolve()
    for parent in current.parents:
        modules_dir = parent / "specs" / "modules"
        if modules_dir.exists():
            return parent
    # Fallback to legacy heuristic (4 levels up) if nothing matches
